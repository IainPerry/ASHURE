...
def generate_pseudo_refdb(primers, reads, block=10000, fs='500-1000', fpmr_thresh=10, workspace='./pseudo_refdb/', config='-x map-ont'):
    logging.info('generate_pseudo_refdb should not have run')
...
def perform_cluster(df_q, df_d=[], max_iter=1, csize=20, N=2000, th_s=2, th_m=0.9, pw_config='', msa_config='', workspace='./cluster/', track_file='', timestamp=True):
    logging.info('clustering should not have run')    
...
def main():
    ...
    if config['run_prfg']:
        logging.info('prfg should not have run')
    ...
    if config['run_fpmr']:
        logging.info('fpmr should not have run')

    if config['run_trmc']:
        logging.info('trmc should not have run')

    if config['run_clst']:
        logging.info('clst should not have run')
